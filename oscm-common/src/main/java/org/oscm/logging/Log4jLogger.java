...
public class Log4jLogger {
  ...
  Log4jLogger(Class<?> category, Locale locale) {
    this.locale = locale;
    systemLogger = LogManager.getLogger(SYSTEM_LOG_CATEGORY_SUFFIX + "." + category.getName());
    accessLogger = LogManager.getLogger(ACCESS_LOG_CATEGORY_SUFFIX + "." + category.getName());
    auditLogger = LogManager.getLogger(AUDIT_LOG_CATEGORY_SUFFIX + "." + category.getName());
    proxyLogger = LogManager.getLogger(REVERSEPROXY_LOG_CATEGORY_SUFFIX + "." + category.getName());
  }

  /**
   * Logs an information with log type . The information is written to the log files specified.
   *
