...
/**
 * This utility class provides the logging functionality to be used by the entire ADM UE
 * application.
 *
...
public class Log4jLogger {
  ...
  /** Indicates that the log information should be written to the system.log file. */
  public static final int SYSTEM_LOG = 1;

  /** Indicates that the log information should be written to the access.log file. */
  public static final int ACCESS_LOG = 2;

  /** Indicates that the log information should be written to the audit.log file. */
  public static final int AUDIT_LOG = 4;

  /** Indicates that the log information should be written to the reverseproxy.log file. */
  ...
  public static final ConfigurationBuilder<BuiltConfiguration> configurationBuilder =
      ConfigurationBuilderFactory.newConfigurationBuilder();

  /**
   * Initializes the logger to write to the console, so that even in case the configuration does not
   * contain correct settings the log information is not lost.
   */
  Log4jLogger(Class<?> category, Locale locale) {
    this.locale = locale;

    systemLogger =
        (Logger) LogManager.getLogger(SYSTEM_LOG_CATEGORY_SUFFIX + "." + category.getName());
    //        systemLogger.setAdditivity(false);
    accessLogger =
        (Logger) LogManager.getLogger(ACCESS_LOG_CATEGORY_SUFFIX + "." + category.getName());
    //        accessLogger.setAdditivity(false);
    auditLogger =
        (Logger) LogManager.getLogger(AUDIT_LOG_CATEGORY_SUFFIX + "." + category.getName());
    //        auditLogger.setAdditivity(false);
    proxyLogger =
        (Logger) LogManager.getLogger(REVERSEPROXY_LOG_CATEGORY_SUFFIX + "." + category.getName());
    //        proxyLogger.setAdditivity(false);
  }

  /**
   * Logs an information with log type {@link Level#INFO}. The information is written to the log
   * files specified.
   *
   * @param logTargets The target log files to be used.
   * @param identifier The message identifier to be logged.
   * @param params The parameters for the place holders of the message
   */
  public void logInfo(int logTargets, LogMessageIdentifier identifier, String... params) {
  ...
  /**
   * Logs information with log type {@link Level#DEBUG}. The information is written to the
   * system.log file.
   *
   * @param message The message to be logged.
   */
  ...
  /**
   * Logs an information with log type {@link Level#WARN}. The information is written to the log
   * files specified.
   *
   * @param logTargets The target log files to be used.
   * @param identifier The message identifier to be logged.
   * @param params The parameters for the place holders of the message
   */
  public void logWarn(int logTargets, LogMessageIdentifier identifier, String... params) {
    String message = getLogMessageText(identifier, params);
    if (logToSystemLog(logTargets)) {
  ...
  /**
   * Logs information with log type {@link Level#ERROR}. The information is written to the
   * system.log file.
   *
   * @param identifier The message identifier to be logged.
   * @param params The parameters for the place holders of the message
   ...
   *
   * @param logTargets The target log files to be used.
   * @param e Throwable to be listed in the log message.
   * @param identifier The message identifier to be logged.
   * @param params The parameters for the place holders of the message
   */
  public void logWarn(
      int logTargets, Throwable e, LogMessageIdentifier identifier, String... params) {
    String message = getLogMessageText(identifier, params);
    if (logToSystemLog(logTargets)) {
  ...
  /**
   * Logs an information with log type {@link Level#ERROR}. The information is written to the log
   * files specified.
   *
   * @param logTargets The target log files to be used.
   * @param e Throwable to be listed in the log message.
   ...
   *
   * @return <code>true</code> in case debug logging is enabled, <code>false</code> otherwise.
   */
  public boolean isDebugLoggingEnabled() {
    return systemLogger.isDebugEnabled();
  }

  protected String getLogMessageText(LogMessageIdentifier identifier, String... params) {
    String text;
    try {
  ...
  private ResourceBundle getResourceBundle() {
    ...
    try {
      bundleForLog = ResourceBundle.getBundle(LOG_MESSAGE_RESOURCE_BASENAME, locale);
    } catch (MissingResourceException e) {
      bundleForLog = ResourceBundle.getBundle(LOG_MESSAGE_RESOURCE_BASENAME);
    }
    return bundleForLog;
  }
